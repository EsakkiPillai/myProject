 ...
 */
public class nRelateDataLoader implements CassandraUtils{
    ...
    public void insert(String filename) {
        String customField = "";
        String customValue ="";
        if(tableName.equals(WIDGET_TABLE))   {
            customField = "widgetid";
        }else if (tableName.equals(PLATFORM_TABLE)){
            customField = "platform";
        }else if(tableName.equals(COUNTRY_TABLE)){
            customField = "country";
        }
        PreparedStatement ps = session.prepare("INSERT INTO "+tableName+" (domain,pixeldate,"+customField+",impressions,paidimpressions,ads,internals,externals,gross_revenue,net_revenue,gross_rpm,net_rpm,ad_ctr,internal_ctr) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                ...
                if (parse(cassandraProperties,line, lineNumber++)) {
                    try{
                        ...
                        if(tableName.equals(WIDGET_TABLE))   {
                            ...
                            customValue = cassandraProperties.getPlatform();
                        }else if(tableName.equals(COUNTRY_TABLE)){
                            customValue = cassandraProperties.getCountry();
                        }
                        if(tableName.equals(PLATFORM_TABLE) || tableName.equals(COUNTRY_TABLE)){
                            statement = ps.bind(cassandraProperties.getDomain(), cassandraProperties.getPixeldate(), customValue,cassandraProperties.getImpressions(), cassandraProperties.getPaidimpressions(),cassandraProperties.getAds(), cassandraProperties.getInternals(), cassandraProperties.getExternals(),cassandraProperties.getGrossRevenue(),
                                cassandraProperties.getNetRevenue(), cassandraProperties.getGrossRPM(), cassandraProperties.getNetRPM(), cassandraProperties.getAdCTR(), cassandraProperties.getInternalCTR());
                        }
                        long start = System.currentTimeMillis();
                        //batch.add(statement);
                        session.execute(statement);
                        logger.info("line number "+lineNumber+" date "+cassandraProperties.getPixeldate()+"domain "+cassandraProperties.getDomain());
                    }catch (Exception e){
    ...
    boolean parse(CassandraProperties cassandraProperties,String line, int lineNumber) {
        ...
        try {
            ...
            if(tableName.equals(WIDGET_TABLE)){
                ...
                cassandraProperties.setCountry(values[i++].trim());
            }else if(tableName.equals(PLATFORM_TABLE)){
                cassandraProperties.setPlatform(values[i++].trim());
            }
