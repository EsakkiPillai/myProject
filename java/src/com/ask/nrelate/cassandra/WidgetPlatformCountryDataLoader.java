...
public class WidgetPlatformCountryDataLoader {
            ...
            try{
                while ((line = reader.readLine()) != null) {
                    ...
                    if (parse(cassandraProperties,line, lineNumber++)) {
                        try{
                            statement = ps.bind(cassandraProperties.getDomain(), cassandraProperties.getPixeldate(),lineNumber,cassandraProperties.getWidgetid(),cassandraProperties.getPlatform(), cassandraProperties.getCountry(),cassandraProperties.getImpressions(), cassandraProperties.getPaidimpressions(),cassandraProperties.getAds(), cassandraProperties.getInternals(), cassandraProperties.getExternals(),cassandraProperties.getGrossRevenue(),
                            ...
                            /*if(!domainsSet.contains(cassandraProperties.getDomain())){
                                String query = "delete from "+tableName+" where domain='"+cassandraProperties.getDomain()+"' and pixeldate='2013-12-09'";
                                domainsSet.add(cassandraProperties.getDomain());
                                session.execute(query);
                                System.out.println("DELETING:"+cassandraProperties.getDomain()+" DomainNo:"+ ++deleteCount);
                            }*/
                            //statement = deleteStatement.bind(cassandraProperties.getDomain(),cassandraProperties.getPixeldate());
                            //deleteBatch.add(statement);
                            //session.execute(statement);
                            System.out.println("line number:"+lineNumber+" date:"+cassandraProperties.getPixeldate()+" domain:"+cassandraProperties.getDomain());
                            if(lineNumber%75==0){
                                //session.execute(deleteBatch);
                                //deleteBatch = new BatchStatement();
    ...
    boolean parse(CassandraProperties cassandraProperties,String line, long lineNumber) {
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-M-dd");
