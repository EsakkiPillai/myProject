...
public class CountryDataLoader {
    ...
    private Logger logger = Logger.getLogger(CountryDataLoader.class.getName()) ;
    ...
    public void insert(String filename) {
        PreparedStatement ps = session.prepare("INSERT INTO "+tableName+" (domain,pixeldate,country,impressions,paidimpressions,ads,internals,externals,gross_revenue,net_revenue,gross_rpm,net_rpm,ad_ctr,internal_ctr) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parse(cassandraProperties,line, lineNumber++)) {
                    try{
                        ...
                        statement = ps.bind(cassandraProperties.getDomain(), cassandraProperties.getPixeldate(), cassandraProperties.getCountry(),cassandraProperties.getImpressions(), cassandraProperties.getPaidimpressions(),cassandraProperties.getAds(), cassandraProperties.getInternals(), cassandraProperties.getExternals(),cassandraProperties.getGrossRevenue(),
    ...
    public static void main(String args[]) {
        if(args.length!=3){
            System.out.println("Usage: java CountryDataLoader TableName SeedsSeperatedByComma InputFilePathSeparatedByTab");
            System.out.println("Example: java CountryDataLoader nrelate_data 10.168.6.95,10.31.137.47 /home/nrelate/nrelate_data.tsv");
        ...
        CountryDataLoader widgetDataLoader = new CountryDataLoader();
